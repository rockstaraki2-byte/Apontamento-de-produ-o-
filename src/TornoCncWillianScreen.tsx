...
export function TornoCncWillianScreen({
  ...
  const finalizeLog = (
    ...
    if (isManual) {
      ...
      db.addNotification({
        message: `Torno CNC Willian: ${qty} un. de "${mTitle}" ${parentName ? `p/ produto ${parentName} ` : ""}(${mProcess}) via ${finalOperatorName}`,
        read: false,
      ...
      db.addNotification({
        message: `Torno CNC Willian: ${qty} un. de ${itemName} FINALIZADAS por ${finalOperatorName}${ordersSuffix}`,
        read: false,
        tenantId: db.activeTenantId || currentUser.tenantId || "imperio",
      });
    }
