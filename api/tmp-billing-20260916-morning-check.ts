...
const TOKEN = "sep16-morning-check-8f4b27d3c1a64d1d";
const require = createRequire(import.meta.url);
const firebaseConfigFile = require("../firebase-applet-config.json") as any;
const APP_NAME = "tmp-billing-20260916-morning-check";
...
function tenantMatches(value: any) { return String(value?.tenantId || "imperio") === "imperio"; }
...
export default async function handler(req: any, res: any) {
  ...
  const codes = ["67517","67527","66849","67488","67526","67491","67533"];
  const orders: Record<string, any[]> = {};
  for (const code of codes) {
    orders[code] = ordersAll.filter((o: any) => String(o.orderCode).trim() === code).map((o: any) => {
  ...
  const catalogCodes = ["5528","5529","5530","5531","3829","3118","287","1","9","2459","2739","3191","4811","2806","3873","958","1281","4051"];
  const catalog: Record<string, any[]> = {};
  for (const code of catalogCodes) {
    catalog[code] = items.filter((item: any) => String(item.code ?? item.id).trim() === code || String(item.code ?? "").startsWith(`${code}.`)).map((item: any) => ({ id:item.id, code:item.code, name:item.name, productionPoints:item.productionPoints??null, basePrice:item.basePrice??null }));
  }
