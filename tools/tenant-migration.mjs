...
function classify(collectionName, docId, data) {
  ...
  // O banco de produção possui somente o tenant operacional imperio.
  // Documentos antigos sem marcador de tenant pertencem ao cadastro anterior
  // à introdução do isolamento e são elegíveis para receber somente tenantId.
  return {
    action: "planned",
...
function getServiceAccount() {
  ...
  if (!raw || !raw.trim()) {
    throw new Error(
      "Credencial administrativa não configurada. Defina FIREBASE_SERVICE_ACCOUNT_KEY no GitHub Actions.",
    );
  }

  const parsed = JSON.parse(raw);
  if (!parsed.project_id && !parsed.projectId) {
    throw new Error("A credencial administrativa não informa o projeto Firebase.");
  }
...
async function inspectCollection(db, collectionName) {
  ...
  return { collectionName, counts, planned, rows };
}

async function applyCollection(db, inspected) {
  const { collectionName, counts, planned } = inspected;
  ...
  for (const refs of chunk(planned, 100)) {
    ...
    for (const snapshot of verifiedSnapshots) {
      const tenant = normalized(snapshot.data()?.tenantId);
      if (tenant === TENANT_ID) {
        counts.verified += 1;
      } else {
        counts.failures += 1;
...
async function main() {
  ...
  // Leitura em pequenos grupos para reduzir pressão no Firestore sem perder
  // o inventário completo das coleções do aplicativo.
